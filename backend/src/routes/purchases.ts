...
purchasesRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const purchase = await prisma.$transaction(async (tx) => {
      ...
      const party = await tx.party.findUniqueOrThrow({ where: { id: input.partyId } });
      const newBalance = Number(party.balance) + subtotal;
      await tx.ledgerEntry.create({
        data: { partyId: input.partyId, debit: subtotal, credit: 0, balance: newBalance, refType: 'Purchase', refId: created.id },
      });
      await tx.party.update({ where: { id: input.partyId }, data: { balance: newBalance } });
